...
export async function recordRun(uid,username,input){
  ...
  let packGranted=false;if(score>=30&&!p.daily.runPackGranted){p.packs=Math.max(0,Number(p.packs)||0)+1;p.daily.runPackGranted=true;packGranted=true}
...
export async function openPlayerPack(uid,username,input){
  ...
  const reveal=[];for(let x=0;x<3;x++){const i=weightedSticker(),prev=Math.max(0,Number(p.collection[i])||0);p.collection[i]=prev+1;if(prev>=1)p.duplicates[i]=Math.max(0,Number(p.duplicates[i])||0)+1;else p.daily.newUnique++;reveal.push(i)}
  p.xp=Math.max(0,Number(p.xp)||0)+15;p.level=Math.floor(p.xp/100)+1;p.packReceipts[openId]={reveal,xp:15};const ids=Object.keys(p.packReceipts);if(ids.length>20)ids.slice(0,ids.length-20).forEach(id=>delete p.packReceipts[id]);await savePlayer(p);return {player:p,reveal,xp:15,alreadyOpened:false}
}
