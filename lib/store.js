...
export async function openPlayerPack(uid,username,input){
  ...
  const reveal=[];for(let x=0;x<3;x++){let i;do{i=weightedSticker()}while(reveal.includes(i));const prev=Math.max(0,Number(p.collection[i])||0);p.collection[i]=prev+1;if(prev>=1)p.duplicates[i]=Math.max(0,Number(p.duplicates[i])||0)+1;else p.daily.newUnique++;reveal.push(i)}
